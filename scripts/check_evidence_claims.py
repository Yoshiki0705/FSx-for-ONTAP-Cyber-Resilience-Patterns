...
#
# DIVERGED 2026-09-07, and deliberately, because the divergence had to be recorded
# somewhere a reader will look. `_tracked_markdown` was rewritten: it raises when git
# cannot enumerate files instead of scanning zero of them, and it lists untracked
# markdown as well, because a document is untracked at exactly the moment a new claim
# is written into it. Both were reachable here: the new-file case was live in this
# working tree, and a sibling project shipped the git-failure case and passed every
# merge silently for weeks. Port both to the canonical copy and re-run the installer,
# then delete this note. Pinned by scripts/tests/test_evidence_gate_file_list.py,
# which is local for the same reason -- the shared test copy would diverge too.
...
ALLOW = re.compile(r"allow:unverified:\s*\S")
...
def _tracked_markdown() -> list[str]:
    """Every markdown file git can see, minus the draft directories.

    Two failures were closed here, both of the same shape: the file list came back empty
    and an empty list is indistinguishable from a clean tree.

    **git failing is not an empty repository.** When this runs somewhere that is not a
    repository -- a hook process starting in an inherited working directory, an extracted
    copy, a container mount without `.git` -- `git ls-files` exits 128 and prints nothing.
    The scan then examined zero files and reported success. A sibling project shipped the
    same shape and every merge passed silently for weeks, so this raises instead.

    **Untracked is not absent.** The invocation listed tracked files only, so a claim in a
    newly written document passed until the file happened to be committed -- which is
    precisely when a new claim enters a document. `-co --exclude-standard` adds files
    present but not yet added, while still honouring `.gitignore`.

    Returns:
        Repository-relative paths, deduplicated and ordered.

    Raises:
        SystemExit: when git cannot enumerate files, rather than reporting none.
    """
    result = subprocess.run(  # noqa: S603
        ["git", "ls-files", "-co", "--exclude-standard", "*.md"],  # noqa: S607
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=False,
    ...
    if result.returncode != 0:
        raise SystemExit(
            f"FAIL: git could not list files in {_rel(ROOT)} (exit {result.returncode}).\n"
            f"      {result.stderr.strip()}\n"
            "      An empty file list is indistinguishable from a clean tree, so this is a "
            "failure rather than a pass. Run this from inside the repository, or set "
            "EVIDENCE_ROOT to it."
        )
    seen: dict[str, None] = {}
...
def _fingerprint(line: str) -> str:
    ...
    """
    return hashlib.sha1(" ".join(line.split()).encode("utf-8")).hexdigest()[:10]  # noqa: S324 - identity, not security
    ...
    for rel in paths:
        ...
        for number, line in enumerate(lines, start=1):
            ...
            if ABSENCE.search(line) and VENDOR.search(line):
                context = line + "\n" + previous
                if not (LEDGER_REF.search(context) or ALLOW.search(context)):
    ...
    for path, hits in sorted(found.items()):
        ...
        for number, fingerprint, text in hits:
            ...
            problems.append(
                ...
                "`allow:unverified: <reason>` when it is deliberately speculative."
            )
