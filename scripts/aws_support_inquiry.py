...
def cmd_draft(entry: dict) -> int:
    ...
    Returns:
        0.
    """
    print(f"SUBJECT: {SUBJECT_PREFIX} {entry['claim'][:120]}")
    print("-" * 72)
    print(build_body(entry))
    print("-" * 72)
...
def cmd_file(claims: list[dict], entry: dict, detail_file: str | None, confirm: bool) -> int:
    ...
    result = client.create_case(
        subject=f"{SUBJECT_PREFIX} {entry['claim'][:120]}",
        ...
        serviceCode=inquiry.get("service_code", DEFAULT_SERVICE_CODE),
        severityCode="low",
        categoryCode="other",
...
def cmd_list() -> int:
    """List the cases this tool opened, by subject prefix.

    Returns:
        0.
    """
    client = _client()
    cases = client.describe_cases(includeResolvedCases=True, maxResults=100).get("cases", [])
    mine = [c for c in cases if c.get("subject", "").startswith(SUBJECT_PREFIX)]
    if not mine:
        print("no cases opened by this tool")
        return 0
    for case in mine:
        print(f"{case['status']:22s} {case['timeCreated'][:10]}  {case['subject'][len(SUBJECT_PREFIX) :].strip()[:80]}")
    return 0
